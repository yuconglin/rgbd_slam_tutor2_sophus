...
bool PoseGraph::tryInsertKeyFrame(RGBDFrame::Ptr& frame)
{
    if ( keyframes.size() == 0 )
    {
        ...
        v->setId( frame->id );
        v->setEstimate( rgbd_tutor::Converter::SE3toSE3Quat(frame->T_f_w.inverse()) );
        v->setFixed(true);
    ...
    SE3d delta = frame->getTransform().inverse() * refFrame->getTransform();
         ...
         norm_rotate( delta ) > keyframe_min_rotation )
    {
        ...
        g2o::VertexSE3* v = new g2o::VertexSE3();
        v->setId( frame->id );
        v->setEstimate( rgbd_tutor::Converter::SE3toSE3Quat(frame->getTransform().inverse()) );
        v->setFixed(false);
        optimizer.addVertex( v );
        vertexIdx.push_back( frame->id );
        keyframes.push_back( frame );
...
void PoseGraph::mainLoop()
{
    ...
    while(1)
    {
        ...
        boost::timer timer;
        vector<RGBDFrame::Ptr>  newFrames_copy;
        {
            // 复制一份newFrames，防止处理的时候有新的东西插入
            unique_lock<mutex> lck(keyframes_mutex);
            newFrames_copy = newFrames;
            newFrames.clear();
        }
        ...
        for ( auto nf : newFrames_copy )
        {
            ...
            for ( int i=0; i<nearbyFrames; i++ )
            {
                ...
                edge->setMeasurement( rgbd_tutor::Converter::SE3toIsometry(info.T) );
                edge->setInformation( Eigen::Matrix<double,6,6>::Identity() * 100);
                edge->setRobustKernel( new g2o::RobustKernelHuber() );
        ...
        if ( loopAccumulatedError > loopAccuError )
        {
            ...
            for ( auto kf : keyframes )
            {
                ...
                if ( v )
                {
                    kf->setTransform( SE3d(v->estimate().inverse().rotation(), v->estimate().inverse().translation()) );
                }
            }
        ...
        else if ( localAccumulatedError > localAccuError )
        {
            ...
            for ( int i=keyframes.size()-1; i>0 && i>keyframes.size()-6; i-- )
            {
                ...
                if ( v )
                {
                    keyframes[i]->setTransform( SE3d(v->estimate().inverse().rotation(), 
                                                     v->estimate().inverse().translation()) );
                }
