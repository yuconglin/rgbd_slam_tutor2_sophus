...
bool rgbd_tutor::PnPSolver::solvePnP( const vector<cv::Point2f>& img, const vector<cv::Point3f>& obj, const rgbd_tutor::CAMERA_INTRINSIC_PARAMETERS& camera, vector<int>& inliersIndex, SE3d& transform )
{
    // g2o初始化
    ...
    {
        vSE3->setEstimate( rgbd_tutor::Converter::SE3toSE3Quat( transform ) );
    ...
    transform = SE3d (se3_recov.rotation(), se3_recov.translation());
...
bool rgbd_tutor::PnPSolver::solvePnPLazy( const rgbd_tutor::RGBDFrame::Ptr & frame1, const rgbd_tutor::RGBDFrame::Ptr frame2, PNP_INFORMATION& pnp_information, bool drawMatches )
{
    ...
    SE3d   init_transform = frame1->T_f_w.inverse() * frame2->T_f_w;
